...
/**
	...
	public class ScaleBox extends UIComponent {
		...
		private var boxScale:Number;//长宽比例

		// 作为scaleBox的内容用的, 测试发现 用graphics 填充,不能很好的触发 over out事件
		private var hitSprite:Sprite;

        private var pGraphics:Graphics;

        [Embed(source="../css/size1.png")]
        private var imageCursor:Class;

		public function ScaleBox(boxWidth:Number, boxHeight:Number, is8dot:Boolean, boxStyle:String, pGraphics:Graphics) {
			super();
            this.pGraphics = pGraphics;
			initUI();
			this.boxScale=boxWidth/boxHeight;
		...
		private function mouseOverHandler(e:MouseEvent):void {
			if (e.target is Dot)
			{
                CursorManager.setCursor(imageCursor,CursorManagerPriority.HIGH, -8, -8);
			}
			else
			{
...
//			removeMouseCursorHandler();
		...
		private function mouseMoveHandler(e:MouseEvent):void
		{
			...
			if (isMove)
			{
                ...
                maskImage(_boxWidth, _boxHeight);
			}
			// 缩放
			else
			{
				...
				*/
				////////////////////////////保持比例
				if(w/h>this.boxScale){
					w=h*this.boxScale;
				}else if(w/h<this.boxScale){
					h=w/this.boxScale;
				}
				///////////////////////////
				resize(w, h);
		...
		private function drawline(w:Number, h:Number):void
		{
			...
			hitSprite.graphics.lineStyle(1, 0xFFFFFF, 1);
			hitSprite.graphics.beginFill(0xFFFFFF, 0);
			if(this.boxStyle == "YuanJiaoJuXing")
			{
				hitSprite.graphics.drawRoundRect(0, 0, w+1, h+1, 16, 16);
			}
			else
			{
				hitSprite.graphics.drawRect(0, 0, w+1, h+1);
			}
    ...
    public function maskImage(w:Number, h:Number):void {
        //从beginFill开始填充颜色，封闭路径内 不填充 所以得到一个中间镂空的遮罩层
        if (parent != null) {
            pGraphics.clear();
            pGraphics.lineStyle(0);
            pGraphics.beginFill(0x000000, 0.4);//背景颜色和透明度
            pGraphics.drawRect(0, 0, parent.width, parent.height);  //以舞台宽高画背景
            pGraphics.lineStyle(1, 0x3399CC); //画一个镂空的框
            pGraphics.drawRect(x-1, y-1, w + 3, h + 3);//画一个封闭路径
            pGraphics.endFill(); //结束填充
        }
    }
